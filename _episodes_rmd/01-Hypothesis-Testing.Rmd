...
- "How do I use continuous, ordinal and categorical data?"
- "What are independent and dependent variables?"
objectives:
- "Understand the difference between correlation and regression"
- "Recognise what data is suitable for correlation/regression analysis"
...
In research, it is extremely common to want to understand whether two groups of
measurements, or variables, have a relationship to each other. Not only that, 
but if there is a relationship, we typically want to study that relationship 
further to identify possible causes and effects, and whether we can use it to 
make further predictions. 

#This workshop will introduce you to some of the different types of relationships
#between data, how to mathematically characterise those relationships,

In this workshop, we will be focussing on two specific types of relationships in
data - _correlation_ and _regression_. We will also be using only ordered 
numerical data; this can be either _continuous data_ or _ordinal data_. 
Continous data is where a variable can take any value on a scale, although there
may be a maximum and minimum to that scale; an example of continuous data is 
height, which has a minimum of zero, a maximum depending on the particular item 
we are measuring the height of, and can take any value between the two. Ordinal 
data is also on a scale, but unlike continuous data it can only be one of a 
defined number of categories; when you fill in a survey which asks for your age 
range (0-15, 16-25, 26-39, and so on), age is being recorded as an ordinal 
value.
